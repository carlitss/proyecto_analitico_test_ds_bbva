...
class FeatureEngineeringTransformer:
    ...
    def create_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df['transactions_count'] = df.groupby('customer_id')['transaction_date'].transform('count')
        ...
        df['amount_abs_moving_avg_3t'] = (df.groupby('customer_id')['amount_abs'].rolling(window=3, min_periods=1).mean().reset_index(level=0, drop=True))
